...
{{-- Note: Uses userDropdownOpen from parent x-data in navigation.blade.php --}}
<div class="w-full">

    {{-- Notification Badge - SEPARATE from button --}}
    @php
        $unreadCount = 0;
        try {
    ...
    @if($unreadCount > 0)
        <a href="{{ Route::has('communications.notifications') ? route('communications.notifications') : '#' }}"
           class="flex items-center justify-between px-2 py-2 mb-2 text-sm text-gray-700 bg-red-50 rounded-md hover:bg-red-100">
            <span class="flex items-center">
                <svg class="w-5 h-5 text-red-500 mr-2" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" />
                </svg>
                Notifications
            </span>
            <span class="px-2 py-0.5 text-xs font-bold text-white bg-red-600 rounded-full">{{ $unreadCount }}</span>
        </a>
    @endif
    ...
    <button @click="userDropdownOpen = !userDropdownOpen"
        ...
        <div class="flex items-center">
            ...
            {{-- User Avatar --}}
            <div class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-indigo-500 flex-shrink-0">
    ...
    <div x-show="userDropdownOpen"
        ...
        @if(Route::has('profile.notification-preferences') && in_array(auth()->user()->role, ['admin', 'manager', 'landlord']))
        ...
        {{-- User Management (Admin only) - Coming Soon --}}
        ...
        {{-- Organization Settings (Admin/Manager only) - Coming Soon --}}
