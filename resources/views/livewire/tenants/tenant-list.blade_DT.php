                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($tenants as $tenant)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{-- @if($tenant->activeLeases->count() > 0)
                                    @foreach($tenant->activeLeases as $lease)
                                        <div class="text-sm text-gray-900">{{ $lease->unit->property->name }}</div>
                                        <div class="text-sm text-gray-500">Unit {{ $lease->unit->unit_number ?? 'N/A' }}</div>
                                        <div class="text-sm text-gray-500">${{ number_format($lease->rent_amount, 0) }}/month</div>
                                    @endforeach
                                @else
                                    <span class="text-sm text-gray-500 italic">No active lease</span>
                                @endif --}}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{-- @if($tenant->activeLeases->count() > 0)
